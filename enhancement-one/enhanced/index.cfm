<!--- ================================================================================
     ...
     02/14/2026 - Updated current/open distribution logic to use status/date, added registration window state display, scoped registration counts to the
                  current distribution, and added unread contact notice + chart updates.
     03/14/2026 - Added User Manager button (Global Admin only) to top nav, and access-denied alert with redirect for unauthorized direct-access attempts.
     ================================================================================ --->
...
<cfparam name="url.msg" default="">                                                                             <!--- Status message token passed via URL — used for access-denied alert display --->
...
<body>
    ...
    <div class="card">                                                                                          <!--- Main admin dashboard card containing stats, status, and charts --->
      <div class="top">                                                                                         <!--- Top bar with dashboard identity and primary navigation actions --->
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">                                                   <!--- Right side of top bar: quick links for admin workflows --->
          ...
          <cfif session.admin.roleName EQ "Global Admin">                                              <!--- User Manager button is restricted to Global Admin accounts only --->
            <a class="btn" href="users.cfm">User Manager</a>
          </cfif>
  ...
  <script>
    <cfif url.msg EQ "accessdenied">                                                                            <!--- Fire access-denied alert when a non-Global Admin attempted direct page access --->
    alert("Access Denied. You do not have permission to access that page.");
    window.location.replace("index.cfm");                                                                       <!--- Strip the msg param from the URL after the user dismisses the alert --->
    </cfif>
